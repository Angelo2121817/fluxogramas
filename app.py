            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        # --- LÓGICA DE EXTRAÇÃO ROBUSTA (PLANO A e PLANO B) ---
                        codigo_dot = None
                        
                        # PLANO A: Tenta achar entre crases (
```)
                        padrao_regex = r"""
```(?:dot)?\s*(.*?)
```"""
                        ...
                        # PLANO B: Se não achar crases, verifica se parece código DOT e pega tudo
                        elif "digraph" in texto:
                            codigo_dot = texto.strip()
                            # Remove possíveis crases soltas no início ou fim se sobrarem
...
```", "")
                        ...
                        if codigo_dot:
                            ...
                            st.warning("O modelo respondeu, mas não consegui identificar o código DOT.")
                            st.write(texto)
